"""Shared display helpers for corporation ESI contracts (freight, buyback, etc.)."""
...
def resolve_location_names(location_ids):
    """Bulk-resolve location IDs to short names when available."""
    if not location_ids:
        return {}
...
def acceptor_display(char, chars_by_user_id=None):
    """Resolve acceptor EveCharacter → User → primary character for display.

    Falls back to the acceptor character when it has no linked user.
...
completed_by_display = acceptor_display


def corp_display_name(corporation_id: int, fallback: str) -> str:
    corp = (
