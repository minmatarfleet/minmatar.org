"""
Pulse/Market simple SIG → Tribe migration executor (writes to default DB).

Applies the migration list produced by pulse_simple_sig_tribe_migration_report.py:
creates active TribeGroupMembership with the user's primary character (one per
entry). tribes.signals add the user to the correct auth.Group. Uses SigRequest
for approved_at/approved_by when available (only Tournaments has a Sig).

Usage (Django runscript, from backend/):
    python manage.py runscript pulse_simple_sig_tribe_migration_execute

Loads migration list from scripts/outputs/pulse_simple_migration_list.json
...
# TribeGroup name → SIG auth.Group name (for SigRequest lookup). Only Tournaments has a Sig.
TRIBE_GROUP_TO_SIG_GROUP = {
    "Tournaments": "SIG - Tournaments",
}
...
def run_sig_to_tribe_migration(
    ...
    """
    Execute the Pulse/Market simple SIG-to-tribe migration.

    Parameters
    ----------
    migration_list:
        List of dicts produced by pulse_simple_sig_tribe_migration_report.py, each with keys:
            user_id (int), tribe_group_id (int), character_ids (list[int]).
        character_ids is a single primary character ID per entry.
    approved_by_user:
...
def run():
    ...
    scripts/outputs/pulse_simple_migration_list.json and runs the migration.
    """
    ...
    if not os.path.isfile(list_path):
        ...
        print(
            "Run the report first: python manage.py runscript pulse_simple_sig_tribe_migration_report"
        )
        return
