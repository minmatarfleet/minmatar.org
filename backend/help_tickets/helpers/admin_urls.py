"""URL builders for help tickets admin custom views."""
...
def changelist_url(model_name: str, **filters: str | int) -> str:
    url = reverse(f"admin:help_tickets_{model_name}_changelist")
    if not filters:
        return url
...
def add_url(model_name: str, **initial: str | int) -> str:
    url = reverse(f"admin:help_tickets_{model_name}_add")
    if not initial:
        return url
