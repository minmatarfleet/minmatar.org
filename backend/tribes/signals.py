...
def tribe_group_membership_post_save(sender, instance, **kwargs):
    """
    Sync auth.Group membership whenever TribeGroupMembership status changes.

    - approved  → add user to tribe_group.group AND parent tribe.group
    - left / removed → remove user from tribe_group.group;
    ...
    if instance.status == TribeGroupMembership.STATUS_APPROVED:
        ...
        TribeGroupMembership.STATUS_LEFT,
        TribeGroupMembership.STATUS_REMOVED,
    ):
        if tribe_group.group:
        ...
        # Remove from the tribe-level group only if the user has no other active memberships
        # in any TribeGroup belonging to this tribe.
        has_other_active = (
