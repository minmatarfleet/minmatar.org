...
type_ids = [
    41490, 32006, 32014, 21894, 21896, 21898, 21902, 21904, 21906, 21918, 21922, 21924, 31932, 31928,
    31924, 31944, 41212, 41215, 41218, 33157, 29336, 17713, 72811, 31890, 31888, 31892, 31894
]
...
@router.get(
    "/csv",
    description="Get LP store item data in CSV format"
)
def send_csv(request):
    ...
    for item in items:
        writer.writerow([item.id, item.name, item.qty_1d, item.qty_7d, item.qty_30d])
...
def update_traded_quantity(item_id):
    url = base_url + '/markets/'+region_forge+'/history/?datasource=tranquility&type_id=' + str(item_id)
    response = requests.get(url)
    total_1d = 0
    total_7d = 0
    total_30d = 0
