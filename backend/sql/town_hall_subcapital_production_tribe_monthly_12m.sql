-- Subcapital production tribe: delivered units and ISK estimates by calendar month, last 12 months.
-- Roster + orders tagged to `Subcapital Production` under tribe slug `industry`.
-- “Delivered” timestamp: COALESCE(assignment.delivered_at, order.fulfilled_at).
-- See town_hall_subcapital_production_tribe_30d.sql for committed/delivered definitions.
WITH params AS (
    SELECT 'industry' AS tribe_slug,
        'Subcapital Production' AS tribe_group_name
),
roster AS (
