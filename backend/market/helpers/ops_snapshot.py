...
# Chart-friendly cap when requesting a multi-day window (≈ hourly over ~3d,
# or one point every ~10h over 30d).
        ...
        cleaned.append(
            {
                ...
                "viable_quantity": int(row.get("viable_quantity", 0)),
...
def _problem_sell_gaps(rows: list[dict]) -> list[dict]:
    """Keep critical coverage/viability gaps only for snapshot storage."""
    return [
        {
            "location_id": row["location_id"],
            "location_name": row["location_name"],
        ...
        for row in rows
        if row.get("coverage_gap") or row.get("viability_gap")
    ]
    ...
    *,
    ...
    for loc in locations:
        ...
        EveMarketOpsMonitorSnapshot.objects.create(
            ...
            contracts_viability_pct=summary.get("contracts_viability_pct"),
            ...
            contract_viable_fulfilled=summary.get(
...
def _serialize_ops_monitor_snapshot(
    snap: EveMarketOpsMonitorSnapshot,
) -> dict:
    return {
        "id": snap.id,
        ...
        "contracts_viability_pct": snap.contracts_viability_pct,
...
def _downsample_snapshots_asc(
    snaps: list[EveMarketOpsMonitorSnapshot],
    max_points: int,
) -> list[EveMarketOpsMonitorSnapshot]:
    """Evenly sample ascending snapshots; always keep oldest and newest."""
    if max_points < 1:
        return []
    n = len(snaps)
    if n <= max_points:
...
def list_ops_monitor_snapshots(
    *,
    ...
    When ``days`` is set, returns snapshots from that window downsampled to
    at most ``max_points`` (for charting). Otherwise returns the newest
    ``limit`` rows (capped).
    """
    ...
    if days is not None:
        ...
        snaps_asc = list(
            qs.filter(captured_at__gte=since).order_by("captured_at")
        )
        snaps_asc = _downsample_snapshots_asc(snaps_asc, max_points)
        snaps_asc.reverse()
        return [_serialize_ops_monitor_snapshot(snap) for snap in snaps_asc]

    limit = max(1, min(limit, HISTORY_LIMIT_MAX))
    return [
        _serialize_ops_monitor_snapshot(snap)
