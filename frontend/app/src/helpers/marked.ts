...
export function getRenderer(open_in_new_tab: boolean = false) {
	...
	renderer.link = (href, title, text) => {
		...
		return localLink && !open_in_new_tab ? html : html.replace(/^<a /, `<a ${replace} `)
	}

	return renderer
}

export const renderer = getRenderer()