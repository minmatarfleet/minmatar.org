...
describe('market_ops_redirect', () => {
    ...
    it('detects stale deep-link params', () => {
        ...
        expect(has_ops_deep_link_params(new URLSearchParams({ location_id: String(AMAMAKE_LOCATION_ID) }))).toBe(true)
        expect(has_ops_deep_link_params(new URLSearchParams())).toBe(false)
    })

    it('requires canonicalization to strip location/doctrine params', () => {
        expect(
            ops_params_need_canonicalization(
                new URLSearchParams({
                    location_name: 'Amamake - 5 times nearly AT winners',
        ...
        expect(ops_params_need_canonicalization(new URLSearchParams())).toBe(false)
    })

    it('builds clean ops redirect paths per tool', () => {
        const translatePath = (path: string) => path
        expect(ops_redirect_path(translatePath, 'monitor')).toBe(OPS_PATHS.monitor)
        expect(ops_redirect_path(translatePath, 'contracts')).toBe(OPS_PATHS.contracts)
        expect(ops_redirect_path(translatePath, 'sell_orders')).toBe(OPS_PATHS.sell_orders)
        expect(ops_redirect_path(translatePath)).toBe('/market/ops/')
    })

    it('maps legacy deep links to the matching clean tool path', () => {
        const translatePath = (path: string) => path
        const amamake = new URLSearchParams({ location_id: String(AMAMAKE_LOCATION_ID) })

        // Old bookmarks kept the tool path; only the location query is stripped.
        expect(ops_redirect_path(translatePath, 'monitor')).toBe('/market/ops/')
        expect(ops_redirect_path(translatePath, 'contracts')).toBe('/market/ops/contracts/')
        expect(ops_redirect_path(translatePath, 'sell_orders')).toBe('/market/ops/sell_orders/')

        // /market?location_id=… and /market/jobs → ops monitor
        expect(ops_redirect_target(translatePath, locations, amamake)).toBe('/market/ops/')
        expect(ops_redirect_target(translatePath)).toBe('/market/ops/')
    })
