...
| Bot kicked FL33T people out of Fishermen Discord | `_kick_stray_role_holders` treated `member_role_id` as exclusive and kicked anyone with that role who lacked a seat. Binding used **Minmatar Fleet Alliance** (shared with FL33T guests in public/external). Exclusive role is **Fisherman** (Secure). Dump `TribeExternalGuild.member_role_id` vs Discord role names, seat statuses (`present` vs `pending_join`), and `TribeGroupMembership` active count. Reconciler must not guild-scan-kick; only unentitled **seats** are kicked. Heal `member_role_id` to Fisherman. Local/dev bots may also have Kick in that guild — audit log shows which bot. Already-pending seats need a one-shot join DM after deploy. |
