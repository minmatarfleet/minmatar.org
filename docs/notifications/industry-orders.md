...
| Key | Mechanism | Who | When |
|-----|-----------|-----|------|
| `industry.order.created` | Broadcast | Last-30-day order participants ∪ topic subscribers (excludes creator) | Order created |
| `industry.order.assignment` | Action-ack | Assigning user | Self-assign to a line |
| `industry.order.job` | External-event | Assignee user | New ESI industry job matched to open assignment |
...
Includes order short code, line qty/type, relevant coordinators, and delivery deep link:

`/industry/orders/contract?order_id=&item_id=&assignment_id=`

## Job matching

On first upsert of an `EveCharacterIndustryJob` during ESI sync:
